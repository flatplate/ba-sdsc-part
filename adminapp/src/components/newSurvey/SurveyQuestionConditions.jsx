...
class SurveyQuestionConditions extends React.Component {
    constructor(props, context) {
        ...
        const indexOfCurrentQuestion = this.props.allQuestions.indexOf(
        ...
        this.defaultInternalCondition = {
            ...
            operator: "is",
            answer: this.possibleConditionQuestions[0].question.answers[0]
                .value,
        };
    ...
    onQuestionChange(event, externalConditionIndex, internalConditionIndex) {
        let newConditions = Array.from(this.state.conditions);
        let condition =
            newConditions[externalConditionIndex].conditions[
    ...
    getCondition(externalConditionIndex, internalConditionIndex, conditions) {
        return conditions[externalConditionIndex].conditions[
            internalConditionIndex
        ];
    }

    onAddInternalCondition(externalConditionIndex, internalConditionIndex) {
        this.state.conditions[externalConditionIndex].conditions.splice(internalConditionIndex, 0, {...this.defaultInternalCondition});
        this.setState({});   
    }
    ...
    setConditions(conditions) {
        this.setState(
            ...
            () =>
                this.props.onConditionsChanged &&
    ...
    deleteCondition(externalConditionIndex, internalConditionIndex) {
        ...
        if (this.state.conditions[externalConditionIndex].conditions.length === 0) {
            let newConditions = Array.from(this.state.conditions).filter(condition => condition !== this.state.conditions[externalConditionIndex]);
        ...
        return (
            <div className="space-y-2">
                {this.state.conditions.map(
                    (externalCondition, externalIndex) => {
                        console.log(externalCondition);
                        return externalCondition.conditions.map(
                            (condition, internalIndex) => (
                                <SurveyQuestionCondition
                                    key={"condition_" + externalIndex + internalIndex}
                                    questions={this.possibleConditionQuestions}
                                    condition={condition}
                                    conditionText={internalIndex === 0 ? "Hide question if" : "and"}
                                    onQuestionChange={(e) =>
                                        this.onQuestionChange(e, externalIndex, internalIndex)
                                    }
                                    onOperatorChange={(e) =>
                                        this.onOperatorChange(e, externalIndex, internalIndex)
                                    }
                                    onAnswerChange={(e) =>
                                        this.onAnswerChange(e, externalIndex, internalIndex)
                                    }
                                    onAddInternalCondition={() => 
                                        this.onAddInternalCondition(externalIndex, internalIndex)
                                    }
                                    onDelete={() => {
                ...
                {this.canHaveConditions ? (
                    <Button
                        textSize="sm"
                        onClick={() =>
                            this.addCondition({ ...this.defaultCondition })
                        }
                    >
                        Add Condition
                    </Button>
                    ...
                    <div className="w-full text-center">
                        This question cannot have conditions
                    </div>
                )}
            </div>
        );
